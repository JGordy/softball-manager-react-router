...
import { Avatar, Card, ColorSwatch, Divider, Group, Image, Text, Title } from "@mantine/core";
...
function FieldPosition({ position, initials, isPreferred, isDisliked }) {
    ...
    return (
        <div className={`${styles.fieldingPosition} ${styles[position.toLowerCase().replace(/\s+/g, "")]}`}>
            <Avatar size="sm" name={initials} alt={position} variant="filled" color={color} autoContrast />
        </div>
    );
}
