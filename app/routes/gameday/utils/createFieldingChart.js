    ...
    for (let inning = 0; inning < innings; inning++) {
        ...
        if (inning !== 0) {
            ...
            outPreviousInning = players.filter(
                (player) => player.positions[inning - 1] === "Out" || countOutPositions(player) >= MAX_OUTS
            );
        }
        ...
        availablePositions?.forEach((position) => {
            ...
            playersCopy.forEach((player) => {
                ...
                if (assignedPlayers.includes(player.firstName + player.lastName) || availablePositions.length === 0)
                    return;
        ...
        const unassignedPlayers = playersCopy.filter(
            (player) => !assignedPlayers.includes(player.firstName + player.lastName)
        );
