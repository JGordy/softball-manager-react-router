...
export default function LineupContainer({ game, managerView, playerChart, players }) {
    ...
    const availablePlayers = players?.filter((p) => p.availability === "accepted");
    const probablePlayers = players?.filter((p) => ["tentative", "noresponse"].includes(p.availability));
    ...
    return (
        <>
            ...
            {managerView && (
                ...
                >
                    <Checkbox.Group value={selectedPlayers} onChange={setSelectedPlayers}>
                        <div mt="xs">
                            {probablePlayers.map((player) => (
                                <Card key={player.$id} p="0" mb="sm">
                                    <Checkbox.Card radius="md" p="sm" value={player.$id}>
