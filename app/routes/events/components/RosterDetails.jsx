...
import { IconClipboardList, IconUsersGroup, IconZoomQuestion } from "@tabler/icons-react";
...
export default function RosterDetails({ deferredData, game, managerView, playerChart, team }) {
                    ...
                    subHeading={
                        ...
                        >
                            {({ attendance, players }) => {
                                ...
                                return (
                                    <Text size="xs" mt="5px" ml="28px" c="dimmed">
                                        {`${documents?.length || 0} responses, ${availablePlayers?.length || 0} ${
                                            availablePlayers?.length === 1 ? "player" : "players"
                                        } available`}
                                    </Text>
                                );
                            }}
            ...
            >
                {playerChart ? (
                    ...
                    <>
                        ...
                        <Text c="dimmed">
                            Lineup and fielding chart for this game has not yet been created.{" "}
                            {managerView ? "As an admin, you can create them below." : "Please check back later."}
                        </Text>
                    </>
                )}
                {managerView && (
